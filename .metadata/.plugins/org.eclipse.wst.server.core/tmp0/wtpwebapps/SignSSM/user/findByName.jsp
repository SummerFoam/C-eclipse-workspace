									...
									<td class="td-status">
										<c:choose>
											...
											<c:when test="${item.type == 2 }">
												<span class="layui-btn layui-btn-danger layui-btn-mini">访客</span>
											</c:when>
										</c:choose>
									</td>
									<td class="td-manage">
										<c:choose>
											...
											<c:when test="${item.type == 1 }">
												<a href="javascript:;" onclick="toTeacher(this,${item.id})" title="修改为学生" style="text-decoration:none;">
						...
						<ul class="pagination">
							<c:choose>
				              <c:when test="${page.pageNow != 1 }"><!-- 如果当前页为1，则不显示首页和上一页 -->
				                  <li><a href="?nickname=${nickname }pageNow=1"><span aria-hidden="true">首页</span></a></li> 
				                  <li><a href="?nickname=${nickname }pageNow=${page.pageNow-1 }" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
				              </c:when>
				         	</c:choose>
							<c:forEach begin="${begin }" end="${end }" var="index">
				              <c:choose>
				                  ...
				                  <c:otherwise><!-- 否则，普通显示 -->
			                  		  <li>
			                  		  	<a href="?nickname=${nickname }pageNow=${index }">${index }</a>
			                  		  </li>
				                  </c:otherwise>
				              </c:choose>
				          	</c:forEach>
				          <c:choose>
				              <c:when test="${page.pageNow != page.totalPageCount }"><!-- 如果当前页为总的记录数，则不显示末页和下一页 -->
				                  <li>
				                  	<a href="?nickname=${nickname }pageNow=${page.pageNow+1 }" aria-label="Next"><span aria-hidden="true">»</span></a>
				                  </li>
				                  <li>
				                  	<li><a href="?nickname=${nickname }pageNow=${page.totalPageCount }" aria-label="Next"><span aria-hidden="true">末页</span></a></li>
				                  </li>
				              </c:when>
				          </c:choose>
				          <li>
