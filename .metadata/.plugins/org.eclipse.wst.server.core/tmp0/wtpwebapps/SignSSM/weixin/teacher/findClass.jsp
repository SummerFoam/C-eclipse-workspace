...
<body>
	...
	<div class="mui-content">
		<div class="title" style="background: #A9CFFF;padding: 5px;color: #fff;">班级列表</div>
			<ul class="mui-table-view">
				<c:forEach items="${list }" var="item">
					<input type="hidden" name="id" value="${item.id }"/>
					<li class="mui-table-view-cell">
						<a class="mui-navigate-right" href="${pageContext.request.contextPath }/student/findStudent?classId=${item.id }">${item.name }</a>
					</li>
				</c:forEach>
		   </ul>
	</div>
